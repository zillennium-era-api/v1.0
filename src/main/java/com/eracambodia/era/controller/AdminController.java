...
public class AdminController {
    ...
    @GetMapping("/users")
    public ResponseEntity listUser(@RequestParam("role")String role,@RequestParam(value="page",defaultValue = "1")int page,@RequestParam(value = "limit",defaultValue = "10") int limit){
        Pagination pagination=new Pagination(page,limit);
        Response response=new Response(200,service.getUsers(role,pagination),pagination);
        return response.getResponseEntity("data","pagination");
    }
    @GetMapping("/user/upgrade_to_agent")
